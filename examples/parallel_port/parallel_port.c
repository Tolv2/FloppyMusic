...
#define DIRECTION_SELECT_PIN_FLOPPY1 0 // Adjust these according to your physical connections.
#define DRIVE_SELECT_PIN_FLOPPY1 1
#define STEP_PIN_FLOPPY1 2
...
#define PARALLEL_PORT_ADDRESS 0x378
uint8_t state = 0x00;

bool midiMode;
void myWriteFunction(int pin, bool value){
    state = (state & ~(1 << pin)) | (value << pin);
    outb(state, PARALLEL_PORT_ADDRESS);
}
...
int main(){
    ...
    if (ioperm(PARALLEL_PORT_ADDRESS,1,1)) {
                printf("Acess denied to %x\n", PARALLEL_PORT_ADDRESS);
                exit(-2);
    }
    printf("Parallel port initialisation succeded.\n");
